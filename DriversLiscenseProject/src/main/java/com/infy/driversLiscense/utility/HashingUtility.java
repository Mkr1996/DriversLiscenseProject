package com.infy.driversLiscense.utility;
...
public class HashingUtility {
	...
	public static String getHashValue(String data) throws NoSuchAlgorithmException {
		...
		for (int i = 0; i < byteData.length; i++) {
			...
			/*
			 * converting each byte data to its corresponding positive number
			 * and storing the hexadecimal format of the same
			 */
			String hex = Integer.toHexString(0xff & byteData[i]);

			/*
			 * if the size of hexadecimal value is one, then appending a '0' in
			 * front of it
			 */
			if (hex.length() == 1)
				hexString.append('0');
